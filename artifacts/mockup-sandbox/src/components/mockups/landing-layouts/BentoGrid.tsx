    ...
    <div className="min-h-screen bg-slate-50 font-['Inter'] text-slate-900 p-4 md:p-8 flex items-center justify-center">
      <div className="max-w-6xl w-full mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[minmax(140px,auto)]">
          ...
          {/* Stats Row - 3 distinct cards (1 heavy, 2 light) spanning 4 cols total: 2-1-1 */}
          ...
          {/* Benefits Grid Area + CTAs embedded */}
          
          {/* Large Benefit 1 - Spans 2 cols, 2 rows */}
          ...
          {/* Small Benefit 1 - Spans 1 col, 1 row */}
          ...
          {/* CTAs Embedded - Spans 1 col, 2 rows */}
          <div className="md:col-span-1 md:row-span-2 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col justify-center items-center text-center gap-4">
            ...
            <p className="text-sm text-slate-500 mb-2">Join thousands of students and teachers today.</p>
            <button className="w-full py-4 px-6 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-medium transition-colors flex items-center justify-center gap-2 group">
              Start Learning
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </button>
            <button className="w-full py-3 px-6 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition-colors">
              Learn More
            </button>
          </div>

          {/* Small Benefit 2 - Spans 1 col, 1 row */}
          <div className="md:col-span-1 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">The algorithm adjusts to your specific forgetting curve.</p>
            </div>
          </div>

          {/* Large Benefit 2 - Spans 4 cols, 1 row (full width bottom) */}
